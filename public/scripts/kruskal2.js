...
function kruskalMaxMin(modo) {
    ...
    edges.forEach(edge => {
        ...
        if (root1 !== root2) {
            ...
            aristasDataSet.update({
                id: edge.id,
                color: { color: modo === 'maximizar' ? 'green' : 'orange' },
                width: 4  // Grosor destacado para las aristas del MST
            });
        }
    });
  ...
  function actualizarResultadosKruskal(mst, modo) {
    ...
    const mensajeLongitud = modo === 'maximizar' ? 'La longitud máxima recorrida es: ' : 'El costo mínimo  de expansión y mantenimiento de la red de antenas es: ';
  
    // Actualizar el contenido del elemento con id 'solucion' en el HTML
    const solucionContainer = document.getElementById('solucion');
    solucionContainer.innerHTML = `<strong>${mensajeLongitud}</strong>${totalLength.toFixed(2)}<br><strong>Costos considerados:</strong> ${camino}`;
  }
